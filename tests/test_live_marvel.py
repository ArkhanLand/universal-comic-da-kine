...
def test_marvel_live_get_pages() -> None:
    ...
    # Acquisition counts assets. Logical pagination is not known from asset order.
    assert pages.interior_image_count == len(sources.pages)
    known_numbers = [n for page in pages.pages if page.numbers is not None for n in page.numbers]
    if pages.logical_page_count is not None:
        assert pages.logical_page_count == len(known_numbers)
    assert len(set(known_numbers)) == len(known_numbers)
    ...
    for page in pages.pages:
        ...
        assert page.numbers is None or len(page.numbers) >= 1
